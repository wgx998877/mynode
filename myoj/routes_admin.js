...
module.exports = function (app){
	...
	app.get('/admin/problem/add', function(req,res){
		res.render ('admin_problem_add', {auto_grade:config.AUTO_GRADING, manual_grade:config.MANUAL_GRADING});
	});
	app.get('/admin/problem/edit/:problem',function (req,res){
		...
		Problem.findOne (query, function (err,problem){
			...
			res.render('admin_problem_edit_problem',{auto_grade: config.AUTO_GRADING,manual_grade:config.MANUAL_GRADING,problem:problem});
		});
	});
	...
	app.post('/admin/problem/add', function(req,res,next){
		...
		for (var i = 1; i<=num_files;i++){
			if (form.grading_type == config.AUTO_GRADING){
				if ((!req.files["infile"+1]) || (!req.files["outfile"+i])){
			...
			if (err){
				...
				if (result){
		            ...
		            if (problem.grading_type == config.AUTO_GRADING){
		                ...
		                basedir = __dirname + '/uploads/problems/' + problem.code;
	...
	app.post ('/admin/problem/grade/*/*',function(req,res){
		...
		ManualGrade.findOne(query,function (err,grade){
			...
			if (grade){
				ManualGrade.update(query,{$set:{score:points}},function (err){
					if (err){
						throw err;
						console.log(err);
						return ;
			...
			if (err){
				...
				User.find({},null,{sort:{name:1}},function (err,users){
					if (err){
						...
						res.render('admin_problem_grade_page',{user:user,problem:problem});
					}
				});
			}
		});
